...
from assets.models import CpuMonit,RemoteUserBindHost
...
class CpuView(ViewSetMixin, APIView):
        ...
        try:
            with transaction.atomic():  # 使用事务
                ...
                if host:
                    ...
                    host_id = host.get("id")
                    # CPU监控表中插入一条记录
                    CpuMonit.objects.create(cpu=cpu,create_time=create_time,time_stamp=time_stamp,host_id=host_id)
                    return HttpResponse(json.dumps(res.__dict__))
                else:
                    res.code = 500
                    res.error = "数据库找不到此ip"
    ...
    def chart_json(self, request,pk, *args, **kwargs):  # 图表数据
        # 读取表所有记录
        queryset = CpuMonit.objects.filter(host_id=pk)
        data = []  # 创建一个空列表
