...
from assets.models import MemoryMonit,RemoteUserBindHost
import json
from rest_framework.permissions import AllowAny
from utils.tools import LegalIP
...
class MemoryView(ViewSetMixin, APIView):
    ...
    def add(self, request, *args, **kwargs):
        ...
        try:
            with transaction.atomic():  # 使用事务
                host = RemoteUserBindHost.objects.filter(ip=host).values("id").first()
                if host:
                    # 获取主机id
                    host_id = host.get("id")
                    # 内存监控表插入一条记录
                    MemoryMonit.objects.create(cur_mem=cur_mem,mem_rate=mem_rate,mem_all=mem_all,create_time=create_time,time_stamp=time_stamp,host_id=host_id)
                    return HttpResponse(json.dumps(res.__dict__))
                else:
                    res.code = 500
                    res.error = "数据库找不到此ip"
    ...
    def chart_json(self, request,pk, *args, **kwargs):  # 图表数据
        # 读取表所有记录
        queryset = MemoryMonit.objects.filter(host_id=pk)
        data = []  # 创建一个空列表
        for i in queryset:  # 遍历，拼横纵坐标
            # 横坐标为时间戳,纵坐标为内存使用率。注意，必须转换类型，否则数据不对。
